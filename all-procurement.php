...
<table class="table">
  ...
  $strSQL = "SELECT * FROM procurement WHERE pcm_id != '' ORDER BY pcm_id DESC LIMIT {$start} , {$perpage}";
  ...
  if($total>0) {
    ...
    while ($objResult=mysqli_fetch_array($objQuery)) {?>
      <tr>
        <td><?php echo $objResult["pcm_id"];?></td>
        <td><?php echo $objResult["pcm_topic"];?></td>
        <td><a href=<?php echo $objResult["pcm_doc"];?>>รายละเอียด</a></td>
        <td><?php echo $objResult["pcm_date"];?></td>
      </tr>
      <?php
      $count++;
    }
...
<?php
$sql2 = "SELECT * FROM procurement WHERE pcm_id != '' ORDER BY pcm_id DESC";
